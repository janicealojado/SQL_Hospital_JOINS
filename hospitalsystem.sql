...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 09, 2026 at 04:05 PM
...
--Patient from Cebu and their appointments
SELECT p.patient_name,
p.city,
a.appointment_name
FROM patients p
...
--Appointments handled by Dr. Garcia
SELECT p.patient_name,
a.appointments.doctor_name,
a.appointments.status
FROM patients p
INNER JOIN appointments a
ON p.patient_id = a.patient_id
...
--All patients and their appointments (include those without appointments)
SELECT p.patient_name,
a.appointment_id
FROM patients p
...
--All appointments even if ptient info is missiing
SELECT appointments.*,
p.patient_name
FROM patients p
